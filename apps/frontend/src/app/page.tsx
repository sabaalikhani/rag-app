...
import { PaperNote } from './api/take-notes/route';
import { QAResponse } from './api/qa/route';
...
export default function Home() {
	...
	>();
	const [notes, setNotes] = useState<Array<PaperNote> | undefined>();
	...
	async function onPaperSubmit(
		...
		const response = await fetch('/api/take-notes', {
			method: 'POST',
			...
			<div className='flex flex-row gap-5 mx-auto mt-3'>
				...
				<div className='flex flex-col gap-2 border-[1px] border-gray-400 rounded-md p-2'>
					<Form {...submitPaperForm}>
						<form
							onSubmit={submitPaperForm.handleSubmit(
								onPaperSubmit
							)}
							className='space-y-8'
						>
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Input
												placeholder='https://arxiv.org/pdf/2401.00400.pdf'
												{...field}
											/>
										</FormControl>
										<FormDescription>
											The URL to the PDF paper you want to
											submit.
										</FormDescription>
										<FormMessage />
									</FormItem>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										...
										<FormControl>
											<Input
												placeholder='Critical point in heavy-quark region of QCD on fine
                        lattices'
												{...field}
											/>
										</FormControl>
